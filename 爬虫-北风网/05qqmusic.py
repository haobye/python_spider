...
# 网址列表可以打印出来，但网址返回为下一行这个
# playlistinfoCallback({"code":0,"subcode":1,"msg":"invalid referer"})
...
for dissid in dissid_list:
    # 返回songmid数据的链接
    url = "https://c.y.qq.com/qzone/fcg-bin/fcg_ucc_getcdinfo_byids_cp.fcg?type=1&json=1&utf8=1&onlysong=0&disstid={0}&format=jsonp&g_tk=1806862358&jsonpCallback=playlistinfoCallback&hostUin=0&format=jsonp&inCharset=utf8&outCharset=utf-8&notice=0&platform=yqq&needNewCode=0".format(
        dissid)
    get_songmid_url_list.append(url)
print(get_songmid_url_list)